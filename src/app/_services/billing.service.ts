...
export class BillingService {
  ...
  getSessionId(data, componentName) {
    const path = apiConstant.BILLING_CREATE_SESSION_ID;
    return this.http.post(environment.apiUrl + path, data).pipe(map(res => res),
      catchError(error => {
        this.onSendLogs(LokiStatusType.ERROR, error, LokiFunctionality.billing, componentName, path);
        return throwError(error);
      }),
    );
  }

  getCurrentPlan(componentName) {
  ...
  createSessionId(componentName) {
    const path = apiConstant.BILLING_UPDATE_SESSION_ID;
    return this.http.get(environment.apiUrl + path).pipe(map(res => res),
      catchError(error => {
        this.onSendLogs(LokiStatusType.ERROR, error, LokiFunctionality.updateBilling, componentName, path);
        return throwError(error);
      }),
    );
  }
  getCurrentPlanInfo(componentName) {
  ...
  upGradePlan(data, componentName) {
